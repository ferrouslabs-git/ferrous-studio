// Static component metadata, default labels, shell families and screen
// patterns. Ported verbatim from legacy/js/data.js; this is the shared
// vocabulary between design and engineering, so names are deliberate.
import { RegionName } from "./model/types";

export type ComponentGroup = "Structure" | "Content" | "Interaction";

export interface ComponentTypeMeta {
  group: ComponentGroup;
  label: string;
  desc: string;
  /** Optional: a new type may declare its own defaults; see getDefaultProps. */
  defaultProps?: Record<string, unknown>;
}

export const COMPONENT_TYPES: Record<string, ComponentTypeMeta> = {
  ...
  "editable-component": {
    ...
    desc: "Reusable custom block editable with the builder",
  },
...
export const PATTERNS: Pattern[] = [
  { id: "empty-screen", icon: "EMP", name: "Empty screen", group: "Starter", desc: "Blank screen with no components", components: [] },
  { id: "nav-basic-only", icon: "N-B", name: "Nav only - Basic", group: "Top Nav", desc: "Header with basic top navigation", components: ["nav-basic"] },
  { id: "nav-search-only", icon: "N-S", name: "Nav only - Search", group: "Top Nav", desc: "Header with search-enabled top navigation", components: ["nav-search"] },
  { id: "nav-cta-only", icon: "N-C", name: "Nav only - CTA", group: "Top Nav", desc: "Header with CTA-style top navigation", components: ["nav-cta"] },
  { id: "left-panel-simple", icon: "L-S", name: "Left panel - Simple", group: "Left Panel", desc: "Main area with simple side navigation", components: ["sidenav-simple", "main"] },
  { id: "left-panel-grouped", icon: "L-G", name: "Left panel - Grouped", group: "Left Panel", desc: "Main area with grouped side navigation", components: ["sidenav-grouped", "main"] },
  { id: "left-panel-workspace", icon: "L-W", name: "Left panel - Workspace", group: "Left Panel", desc: "Main area with workspace switcher side navigation", components: ["sidenav-workspace", "main"] },
  { id: "right-panel-detail", icon: "R-D", name: "Right panel - Detail", group: "Right Panel", desc: "Main area with right-side detail panel", components: ["main", "rightpanel-detail"] },
  { id: "right-panel-filters", icon: "R-F", name: "Right panel - Filters", group: "Right Panel", desc: "Main area with right-side filter panel", components: ["main", "rightpanel-filters"] },
  { id: "right-panel-activity", icon: "R-A", name: "Right panel - Activity", group: "Right Panel", desc: "Main area with right-side activity feed", components: ["main", "rightpanel-activity"] },
  { id: "full-shell-basic", icon: "F-B", name: "Full shell - Basic nav", group: "Full Shell", desc: "Basic nav + simple left panel + detail right panel", components: ["nav-basic", "sidenav-simple", "main", "rightpanel-detail"] },
  { id: "full-shell-search", icon: "F-S", name: "Full shell - Search nav", group: "Full Shell", desc: "Search nav + grouped left panel + filters right panel", components: ["nav-search", "sidenav-grouped", "main", "rightpanel-filters"] },
  { id: "full-shell-cta", icon: "F-C", name: "Full shell - CTA nav", group: "Full Shell", desc: "CTA nav + workspace left panel + activity right panel", components: ["nav-cta", "sidenav-workspace", "main", "rightpanel-activity"] },
];
