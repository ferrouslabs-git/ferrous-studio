...
export type OutboxPhase = "idle" | "syncing" | "retrying";
...
export class Outbox {
  ...
  snapshot(): OutboxSnapshot {
    return {
      ...
      phase: this.inFlight ? "syncing" : this.retryTimer ? "retrying" : "idle",
  ...
  private async drain(): Promise<void> {
    if (!this.ready || this.disposed || this.inFlight || this.retryTimer || this.queue.length === 0) return;
    this.inFlight = true;
